...
class hide_my_site{
            ...
            <div id="green_ribbon">
            	...
            	<div id="green_ribbon_top">
                    ...
                    <div id="green_ribbon_base">
                        ...
                        <span class="hms_get_premium_meta">Hide My Site Premium is now available for as low as $10!</span>
                    </div>
                	...
                	<div class="motech_premium_box_wrap">
                        ...
                        <div class="motech_purchase_buttons">
                            ...
                            <div class="motech_purchase_button one_use">
                                <form action="https://www.paypal.com/cgi-bin/webscr" method="post"><input name="cmd" value="_s-xclick" type="hidden"><input name="hosted_button_id" value="KAVAZUVQNY7AE" type="hidden"><input type="hidden" name="page_style" value="hide_my_site_premium">
                                    <button name="submit">
                            ...
                            <div class="motech_purchase_button three_use">
                                <form action="https://www.paypal.com/cgi-bin/webscr" method="post"><input name="cmd" value="_s-xclick" type="hidden"><input name="hosted_button_id" value="QYK99PLHZEQCS" type="hidden"><input type="hidden" name="page_style" value="hide_my_site_premium">
                                    <button name="submit">
                            ...
                            <div class="motech_purchase_button unlimited_use">
                                <form action="https://www.paypal.com/cgi-bin/webscr" method="post"><input name="cmd" value="_s-xclick" type="hidden"><input name="hosted_button_id" value="LFA2K7JZWABA2" type="hidden"><input type="hidden" name="page_style" value="hide_my_site_premium">
                                    <button name="submit">
    ...
    public function page_init(){
		...
		$field_id = $this->plugin_slug.'_'.$field_slug;
		register_setting($this->plugin_slug.'_option_group', $field_id, array($this, $field_slug . '_v'));
		if (get_option($this->plugin_slug . '_ihmsa','') == 'hmsia') {
			$desc = "<div class='mvalid'>Valid</div>";
		} else {
			$desc = "Enter your license key to unlock premium features. <a href='javascript:void(0)' class='hms_get_premium'>Get Premium »</a>";
		}
		add_settings_field(
		    $field_id,
		    $field_label, 
	...
	function plk_v($input) {
		if (get_option($this->plugin_slug . '_ihmsa','') == 'hmsia') {
			return get_option($this->plugin_slug . '_plk','');	
		}
		$output = "";
		if (!empty($input)) {
			$c = $this->c_l($input);
			if ($c == "vle") {
				$output = $input;
				add_settings_error('plk_error_id3',esc_attr('settings_updated_5'),__('Settings saved.'),'updated');
				add_settings_error('plk_error_id4',esc_attr('settings_updated_6'),__('Your license key is valid. Enjoy the premium features!'),'updated success');
				update_option( $this->plugin_slug . '_ihmsa', 'hmsia' );
			} else {
				$output = "";
				add_settings_error('plk_error_id2',esc_attr('settings_updated_3'),__('Settings saved.'),'updated');
				add_settings_error('plk_error_id',esc_attr('settings_updated_2'),__('The license key you entered is invalid. It either does not exist or has no remaining uses.'),'error');
			}
		} else {
			$output = "";
		}
	...
	function c_l($k) {
		...
		$url = 'http://www.clevelandwebdeveloper.com/hmsl/vl.php?k='.$k.'&w=' . get_bloginfo('url');
			...
			?>
				<script>
					jQuery(function() {
						...
						<?php if (get_option($this->plugin_slug . '_ihmsa','') == 'hmsia') : ?>
						useval = jQuery("#hide_my_site_plk").val();
	...
	function create_a_text_input($args) {
		...
		// Render the output
		echo '<input type="text" '  . $placeholder_html . $max_length_html . ' id="'  . $args["id"] . '" name="'  . $args["id"] . '" value="' . get_option($args["id"], $default) . '" />';
		if($args["desc"]) {
